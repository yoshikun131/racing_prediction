    ...
    try:
        ...
        if dist >950:
            ...
            for elem2 in elems2:
                ...
                if len(dfs) != 2:
                    dfs2 = dfs[1][ ~(dfs[1]['着順'].str.contains('-'))]
                    ...
                    if flag==1:
                        dfc2 = dfs2[( ~(dfs2['着順'].str.contains('-/')) & (dfs2['距離'].str.contains('ダ')) )]
                        dfc2['距離'] = dfc2['距離'].str.lstrip("ダ")
                    else:
                        dfc2 = dfs2[( ~(dfs2['着順'].str.contains('-/')) & (dfs2['距離'].str.contains('芝')) )]
                    ...
                    if len(dfc2) > 0:
                        ...
                        #着順数値化
                        number=dfc['着順']
                        number=number.values.tolist()
                        length=len(number)
                        power=np.zeros(length)
                        for i in range(len(number)):
                            
                            split=number[i].split('/')
                            power[i]=float(split[1])/float(split[0])
                            #number[i]=1/float(Fraction(number[i]))
...
#                        speed[np.isnan(speed)] = 0.0
                        if len(number) < 5:
                            ...
                            mean[r][h] = power[0:5].mean()
                            std[r][h] = power[0:5].std()
                h+=1
